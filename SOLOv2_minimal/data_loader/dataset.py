...
class CustomIns_separate(data.Dataset):
    ...
    def _build_coco_gt(self):
        gts = {
            'img_ids': self.ids,
            'cate_ids': list(range(1, self.cfg.num_classes + 1)),
        ...
        for idx in self.ids:
            ...
            label_path = f'{self.label_folder}/{img_name[:-4]}.json'
            ...
            height, width = content['img_height'], content['img_width']

            mask_list = get_seg_mask(self.cfg.class_names, polygons, height, width)

            for poly, mask in zip(polygons, mask_list):
                category_id = self.cfg.class_names.index(poly['category']) + 1
